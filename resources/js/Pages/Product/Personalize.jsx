...
export default function Personalize({ auth, user, product, sizes, colors }) {
    ...
    const applyFilter = () => {
        ...
        image.src = '/storage/' + selectedImage;
        image.onload = () => {
    ...
    const handleSelectionChange = (type, value) => {
        if (type === "size") {
            ...
            let sleeveImage;
            if (value === "Sin Manga") {
                sleeveImage = product.sleeveless;
            } else if (value === "3/4") {
                sleeveImage = product.quarters;
            } else if (value === "Largo") {
                sleeveImage = product.large;
            } else if (value === "Corto") {
                sleeveImage = product.image;
            }
            if (sleeveImage) {
                setSelectedImage(sleeveImage);
            } else {
    ...
    const getSelectedClass = (type, value) => {
        if (type === "size") {
            ...
            return selectedSleeve === value ? "bg-indigo-600 text-white" : "";
        }
    };
    ...
    useEffect(() => {
        if (selectedSize !== null && selectedColor !== null && selectedSleeve !== null && selectedImage !== null) {
            setAddToCartDisabled(false);
        } else {
            setAddToCartDisabled(true);
        }
    }, [selectedSize, selectedColor, selectedSleeve, selectedImage]);
                ...
                <div className="w-full mx-auto px-4 sm:px-6 lg:px-0 my-6">
                    <div className="grid md:grid-cols-2">
                        <div className="img flex items-left w-96 mx-auto mb-5">
                            <div className="img-box bg-gray-800 h-96 w-full mx-auto my-auto rounded-lg shadow-lg overflow-hidden flex-shrink-5">
                                {selectedImage !== null ? (
                                    <img src={'/storage/'+ selectedImage} alt={selectedImage} className="object-cover mx-auto filter brightness-150" width={350}/>
                                ) : (
                        ...
                        <div className="data w-full xl:justify-start justify-center flex items-center xl:my-2 lg:my-2">
                            <div className="data w-full max-w-xl">
                                    ...
                                    <div className="grid grid-cols-3 min-[300px]:grid-cols-5 gap-3">
                                        {sizes.map(size => (
                                            <label key={size.id} className="inline-flex items-center">
                                                <input type="radio" name="size" value={size.id} className="sr-only" onChange={() => handleSelectionChange("size", size.id)} checked={selectedSize === size.id} />
                                ...
                                <p className="text-gray-900 text-md font-medium">Manga</p>
                                <div className="w-full pb-2 border-b border-gray-100 flex-wrap">
                                    <div className="grid grid-cols-4 gap-3">
                                        {["Corto", "Largo", "Sin Manga", "3/4"].map(sleeve => (
                                            <label key={sleeve} className="inline-flex items-center">
                                                <input type="radio" name="sleeve" value={sleeve} className="sr-only" onChange={() => handleSelectionChange("sleeve", sleeve)} checked={selectedSleeve === sleeve} />
                                                <span className={`bg-indigo text-center py-1.5 px-6 w-full font-semibold text-sm leading-8 text-gray-900 border border-gray-200 flex items-center rounded-full justify-center transition-all duration-300 visited:border-gray-300 visited:bg-gray-50 ${getSelectedClass("sleeve", sleeve)}`}>{sleeve}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-2">
                                    <div className="flex sm:items-center sm:justify-center w-full">
                                        <button onClick={decreaseQuantity} className="group py-4 px-6 border border-gray-400 rounded-l-full bg-white transition-all duration-300 hover:bg-gray-50 hover:shadow-sm hover:shadow-gray-300">
                                            <svg className="stroke-gray-900 group-hover:stroke-black" width="16" height="16"
                                                viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M16.5 11H5.5" stroke="" stroke-width="1.6" stroke-linecap="round" />
                                                <path d="M16.5 11H5.5" stroke-opacity="0.2" stroke-width="1.6"
                                    ...
                                    <button 
                                     ...
                                     disabled={addToCartDisabled}>
                                        <svg className="stroke-indigo-600 " width="22" height="16" viewBox="0 0 22 22" fill="none"
